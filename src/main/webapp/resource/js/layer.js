...
Class.pt.space = function(html){
    ...
    var mainborder = typeof(config.mainborder) == 'undefined' ? '' : config.mainborder;
    var titlebgcolor = typeof(config.titlebgcolor) == 'undefined' ? '' : 'style="'+config.titlebgcolor+';" ';
    ...
    config.title[1] && (title = '<h2 class="xubox_title" '+titlebgcolor+'><em>' + config.title[0] + '</em></h2>')
        
    var boxhtml = '<div times="'+ times +'" showtime="'+ config.time +'" style="z-index:'+ zIndex +'" id="'+ dom.lay +''+ times 
    +'" class="'+ dom.lay +'">'	
    + '<div style="background-color:'+ config.bgcolor +'; z-index:'+ zIndex +'; '+mainborder+'" class="xubox_main">'
    + frame[config.type]
    + title
    + closebtn
...
function alert_success(msg) {
	$.layer({
		dialog:{type:9,msg:msg},
		title:'提示消息',
		success: function(){

		}, end: function(){
			if($('#success_url').val()) {
				location.href=$('#success_url').val();
			}else if($('#list_getpage').val()) {
				list_getpage(1);
			}
		}
	});	
