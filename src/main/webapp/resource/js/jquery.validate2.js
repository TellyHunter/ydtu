...
$.extend($.validator, {
	...
	defaults: {
		...
		onfocusin: function (element, event) { 
			...
			//得到错误信息
			var errorMes = this.addWrapper(this.errorsFor(element)).html();
			//如果错误信息不为空，则返回，不用进入设置提示信息
			if(errorMes != ""){
				return;
			}
...
//			this.addWrapper(this.errorsFor(element)).html("");
			this.addWrapper(this.errorsFor(element))
			.removeClass(this.settings.errorClass)  
...
//			}); 
			...
			$(this.currentForm)
				.validateDelegate(":text, [type='password'], [type='file'], select, textarea, " +
					...
					"[type='range'], [type='color'], [type='phoneCheck'],[type='passRule'],[type='carPlate'] ",
...
//			.addClass(this.settings.validClass).html("输入正确");
	...
	classRuleSettings: {  
		...
		passRule:{passRule:true},
		carPlate:{carPlate:true}
	}, 
	...
	methods: {
		...
		passRule: function(value, element){
			return this.optional(element) || /^\w+$/.test(value);
		},

		phoneCheck: function(value, element){
			var reg = /^(0|86|17951)?(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/;
			return this.optional(element) || reg.test(value);
		},
		
		carPlate:function(value, element){
